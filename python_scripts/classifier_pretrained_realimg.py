...
dataset = DDSM(dataroot, dataroot3, labelroot, 'train', transform=transform,       # use train real healthy and BC
...
tdataset = DDSM(dataroot, dataroot3, labelroot, 'test', transform=transform,       # use train real healthy and BC
               transform2 = transform2)

test_loader = torch.utils.data.DataLoader(tdataset,
                                        batch_size= batch_size, shuffle=True,
                                        num_workers=num_workers, pin_memory=True)
...
print('Test Accuracy of the model on the real images images: {} %'.format(100 * correct / total))
...
classes = ('0', '1')
r18dataiter = iter(test_loader)
r18images, r18labels = r18dataiter.next()[0], r18dataiter.next()[1] 

# print images

#plt.imshow(np.transpose(vutils.make_grid(r18images.to(device), padding=2, normalize=True).cpu(),(1,2,0)))
#print('GroundTruth: ', ' '.join('%5s' % classes[r18labels[j]] for j in range(64)))


# In[24]:


res18outputs = model(r18images.to(device))


# In[25]:


_, res18predicted = torch.max(res18outputs, 1)

#print('Predicted: ', ' '.join('%5s' % classes[res18predicted[j]]
                             # for j in range(64)))
...
r18roc_score =roc_auc_score(r18labels.cpu(), res18predicted.cpu())
r18average_precision = average_precision_score(r18labels.cpu(),res18predicted.cpu() )
r18F1 = f1_score(r18labels.cpu(), res18predicted.cpu(), average='weighted')


# In[27]:


print('ROC AUC:', r18roc_score, 'Average precision:', r18average_precision, 'F1 score:', r18F1)
...
roc_auc = dict()
for i in classes:
    fpr[i], tpr[i], _ = roc_curve(r18labels.cpu(), res18predicted.cpu())
    roc_auc[i] = auc(fpr[i], tpr[i])

# Compute micro-average ROC curve and ROC area
fpr["micro"], tpr["micro"], _ = roc_curve(r18labels.cpu().numpy().ravel(), res18predicted.cpu().numpy().ravel())
...
plt.title('Receiver operating characteristic for Real Images in Resnet50')
plt.legend(loc="lower right")
#plt.show()
fig.savefig('ROC_Real.png')
...
plt.plot(losses, color = 'm',label="Classfication Loss Real Images ")
plt.xlabel("iterations")
plt.ylabel("Loss")
plt.legend()
#plt.show()
fig.savefig('Classification_loss_Real.png')
...
plt.plot(acc, color = 'm', label="Classfication Accuracy Real Images ")
plt.xlabel("iterations")
plt.ylabel("Accuracy")
plt.legend()
#plt.show()
fig.savefig('Classification_accuracy_Real.png')
