...
class DDSM(torch.utils.data.Dataset):             # Notice that masks are in PNG form and mammograms in JPG
    def __init__(self, root1, root2, root3, root4, labelr, split, transform, transform2, transform3):
        ...
        bc_image_l = list(zip(img_list, label_list, label_o_list))
        #print((bc_image_list))   
        bc_image_list = bc_image_l[:2000]
...
plt.legend()
#plt.show()
fig.savefig('Losses-patch-AE-Box-Mask-ciDCGAN200.png')
...
vutils.save_image(real_batch[0].to(device)[:64], 'RealPatch-AE-Box-Mask-ciDCGAN200.png', padding=2, normalize=True)


# In[ ]:


vutils.save_image(img_list[-1], 'FakePatch-AE-Box-Mask-ciDCGAN200.png')


vutils.save_image(real_batch[3].to(device)[:64], 'NoisePatch-AE-Box-Mask-ciDCGAN200.png', padding=2, normalize=True)

path= '../saved_models/model-ci-DCGAN-BC-AdversarialEncDec-patch-MaskInfilling-BCMasks-2chan-Box-Mask-256/'

torch.save(encoder.state_dict(), path + 'encoder-patch-AE-200e-ciDCGAN.pth') #encoder model is for lr=0.2
torch.save(decoder.state_dict(), path + 'decoder-patch-AE-200e-ciDCGAN.pth') #encoder model is for lr=0.2

torch.save(netD.state_dict(), path + 'netD-patch-AE-200e-ciDCGAN.pth')
