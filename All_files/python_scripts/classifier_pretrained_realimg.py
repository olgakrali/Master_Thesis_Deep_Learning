...
# 
# # Real data prior to augmentation
# 
...
class DDSM(torch.utils.data.Dataset):             # Notice that masks are in PNG form and mammograms in JPG
    def __init__(self, root1, root2,  labelr, split, transform, transform2, mode):
        self.root1 = root1
        ...
        if mode == 'train':
            img_list = imagedir[:500]
        elif mode == 'val':
            img_list = imagedir[3000:3400]
        elif mode == 'test':
            img_list = imagedir[3400:]
...
vdataset = DDSM(dataroot, dataroot3, labelroot, 'val', transform=transform,       # use train real healthy and BC
...
optimizer_ft = optim.Adam(params_to_update, lr=1e-05)
...
for epoch in range(num_epochs):  
        ...
        with torch.no_grad():
            for j, data in enumerate(val_loader,0):
                ...
                # Save the model
                    torch.save(model.state_dict(), path + 'bestHealthy.pth') 
